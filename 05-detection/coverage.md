...
**Note for the Author:** Conversion fails with the `azure_monitor` and `sentinel_asim` pipelines, because Windows Security Event 4688 does not record the original file name. Organisations relying only on native Windows auditing cannot detect a renamed vssadmin.exe with this rule; EDR or Sysmon telemetry is required.